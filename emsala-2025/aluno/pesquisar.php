...
$turmaAluno = null;
$resTurma = $conn->query("SELECT ALU_TUR_id FROM aluno WHERE ALU_id = $alunoID");
if ($resTurma && $resTurma->num_rows > 0) {
    $turmaAluno = $resTurma->fetch_assoc()['ALU_TUR_id'];
} else {
    echo "<p style='color:red;'>Erro ao localizar a turma do aluno.</p>";
    exit();
}
?>
        ...
        if ($nomePesq !== '') {
            ...
            if ($tipo == 'todos' || $tipo == 'professores') {
                $res = $conn->query("
                    ...
                    WHERE tpd.TPD_TUR_id = $turmaAluno AND p.PRO_nome LIKE '%$nomePesq%'
                ");
                if ($res->num_rows > 0) {
                    $resultPesquisa = true;
            ...
            if ($tipo == 'todos' || $tipo == 'alunos') {
                $res = $conn->query("SELECT * FROM aluno WHERE ALU_TUR_id = $turmaAluno AND ALU_nome LIKE '%$nomePesq%'");
                if ($res->num_rows > 0) {
                    $resultPesquisa = true;
