        ...
        <div class="content">
            ...
            if ($nomePesq !== '') {
                ...
                if ($tipo == 'todos' || $tipo == 'professores') {
                    $res = $conn->query("SELECT * FROM professor WHERE PRO_nome LIKE '%$nomePesq%'");
                    if ($res->num_rows > 0) {
                        echo "<div class='header header-result'><h2>Professores</h2></div>";
                        echo "<div class='container'>";
                        while ($row = $res->fetch_assoc()) {
                            echo "<div class='card'> <div class='profile-pic'> <img src='img/profile-pic-D.png' alt='Foto de perfil do professor' class='foto-card'> </div> <div class='profile-name'>". htmlspecialchars($row['PRO_nome']) .                         
                            "</div></div>";
                        }
                        echo "</div>";
                    }
                ...
                if ($tipo == 'todos' || $tipo == 'alunos') {
                    $res = $conn->query("SELECT * FROM aluno WHERE ALU_nome LIKE '%$nomePesq%'");
                    if ($res->num_rows > 0) {
                        echo "<div class='header header-result'><h2>Alunos</h2></div>";
                        echo "<div class='container'>";
                        while ($row = $res->fetch_assoc()) {
                            echo "<div class='card'> <div class='profile-pic'> <img src='img/profile-pic-D.png' alt='Foto de perfil do aluno' class='foto-card'> </div> <div class='profile-name'>". htmlspecialchars($row['ALU_nome']) .                         
                            "</div></div>";
                        }
                        echo "</div>";
                    }
                ...
                if ($tipo == 'todos' || $tipo == 'responsaveis') {
                    $res = $conn->query("SELECT * FROM responsavel WHERE RES_nome LIKE '%$nomePesq%'");
                    if ($res->num_rows > 0) {
                        echo "<div class='header header-result'><h2>Responsáveis</h2></div>";
                        echo "<div class='container'>";
                        while ($row = $res->fetch_assoc()) {
                            echo "<div class='card'> <div class='profile-pic'> <img src='img/profile-pic-D.png' alt='Foto de perfil do responável' class='foto-card'> </div> <div class='profile-name'>". htmlspecialchars($row['RES_nome']) .                         
                            "</div></div>";
                        }
                        echo "</div>";
                    }
